...
import PropertyRow from "@/components/site/PropertyRow";
 ...
 type Profile = { name: string; email: string; address?: string; phone?: string };
...
export default function Dashboard() {
  ...
  const [collabs, setCollabs] = useState<Collaborator[]>(() => {
    try { return JSON.parse(localStorage.getItem("collaborators") || "[]"); } catch { return []; }
  });
  const [contactedIds, setContactedIds] = useState<string[]>(() => {
    try { return JSON.parse(localStorage.getItem("contacted") || "[]"); } catch { return []; }
  });

  useEffect(() => {
    const onChange = () => setContactedIds(() => {
      try { return JSON.parse(localStorage.getItem("contacted") || "[]"); } catch { return []; }
    });
  ...
  const contacted = useMemo(() => properties.filter(p => contactedIds.includes(p.id)), [contactedIds]);
    ...
    <div className="bg-white">
      <section className="py-10">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="mt-1 text-muted-foreground">Manage your profile, track contacted rentals, and collaborate.</p>
          ...
          <div className="mt-6">
            <Tabs defaultValue="profile">
                  ...
                  <div className="rounded-xl border bg-white p-6 shadow-sm">
                    ...
                    <form className="mt-4 grid gap-3" onSubmit={(e) => {
                      ...
                      <div className="space-y-2">
                        ...
                        <Input id="targetCity" value={prefs.targetCity || ""} onChange={e => setPrefs({ ...prefs, targetCity: e.target.value })} placeholder="City or neighborhood" />
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor="maxBudget">Max monthly budget ($)</Label>
                        <Input id="maxBudget" type="number" min={0} value={prefs.maxBudget ?? ""} onChange={e => setPrefs({ ...prefs, maxBudget: Number(e.target.value) })} />
                      </div>
                      <div className="pt-2">
              ...
              <TabsContent value="contacted" className="mt-6">
                {contacted.length === 0 ? (
                  <div className="rounded-xl border bg-white p-6 text-center text-muted-foreground">You haven't contacted any rentals yet.</div>
                ) : (
                      ...
                      <div key={p.id} className="relative">
                        ...
                        <div className="absolute right-4 top-4">
                          <Button variant="outline" size="sm" onClick={() => {
                            const next = contactedIds.filter(id => id !== p.id);
                            setContactedIds(next);
                            localStorage.setItem("contacted", JSON.stringify(next));
                  ...
                  <div className="rounded-xl border bg-white p-6 shadow-sm">
                    ...
                    <form className="mt-4 grid gap-3" onSubmit={(e) => {
                      ...
                      <div className="space-y-2">
                        ...
                        <Input id="email" type="email" value={profile.email || localStorage.getItem("userEmail") || ""} onChange={e => setProfile({ ...profile, email: e.target.value })} required />
                      </div>
                      <div className="space-y-2">
                  ...
                  <div className="rounded-xl border bg-white p-6 shadow-sm">
                    ...
                    <form className="mt-4 grid gap-3" onSubmit={(e) => {
                      ...
                      <div className="space-y-2">
                        ...
                        <Input id="pwd-email" type="email" defaultValue={profile.email || localStorage.getItem("userEmail") || ""} required />
                      </div>
                      <div className="pt-2"><Button type="submit">Send reset link</Button></div>
                    </form>
                  </div>
                </div>
              </TabsContent>
              ...
              <TabsContent value="collab" className="mt-6">
                <div className="rounded-xl border bg-white p-6 shadow-sm">
                  ...
                  <form className="mt-4 flex flex-col sm:flex-row gap-2" onSubmit={(e) => {
                    e.preventDefault();
                    const data = new FormData(e.currentTarget as HTMLFormElement);
                    const email = String(data.get("collab-email"));
                    if (!email) return;
                    const next = [...collabs, { email, status: "invited" as const }];
                    setCollabs(next);
                    localStorage.setItem("collaborators", JSON.stringify(next));
                    (e.currentTarget as HTMLFormElement).reset();
                    toast.success("Invitation sent");
                  }}>
                    <Input name="collab-email" type="email" placeholder="collaborator@example.com" required />
