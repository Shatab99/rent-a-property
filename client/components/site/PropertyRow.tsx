    ...
    <div className="group overflow-hidden rounded-xl border bg-white shadow-sm hover:shadow-md transition-shadow flex flex-col sm:flex-row">
      ...
      <div className="flex-1 p-4">
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          <h3 className="font-semibold text-lg">${""}{property.price.toLocaleString()} <span className="text-xs text-muted-foreground font-normal">/mo</span></h3>
          <div className="flex items-center gap-1 text-sm text-muted-foreground"><MapPin size={14} /> {property.location}</div>
        </div>
        ...
        <div className="mt-4">
          <Button asChild><Link to={`/property/${property.id}`}>View details</Link></Button>
        </div>
      </div>
    </div>
  );
